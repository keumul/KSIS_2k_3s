...
int main()
{
	...
	Subnet = IP & Mask; //можно интерпретировать номер подсети как те биты, которые в маске равны 1
	HOST = IP & ~Mask; //в битах, отвечающих за номер подсети, ставим 0
...
unsigned long CharToLong(char* IP_)
{
	...
	{
		if (IP_[i] != '.') //если не точка - записать символ в буфер
			Buffer[j] = IP_[i];
		if (IP_[i] == '.' || IP_[i + 1] == '\0')
		{
